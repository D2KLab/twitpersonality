...
def parseFastText(path, transform=False):
    data = {}
    ...
    for line in dataset:
        ...
        if transform:
            data[word] = np.array(list(float(value) for value in embedding[:-1].split(" ")))
        else:
            data[word] = embedding
...
def readMyPersonalityUserWise_v2():
    ...
    for line in open("Dataset/mypersonality_final.csv", "r"):
        ...
        if old_id == "":
            ...
            y_O_all = float(big5_str[4])
            y_C_all = float(big5_str[3])
            y_E_all = float(big5_str[0])
            y_A_all = float(big5_str[2])
            y_N_all = float(big5_str[1])
        elif old_id != user_id:
            i += 1
            data_all[i] = []
            data_all[i].append(status)
            y_O_all = np.append(y_O_all, float(big5_str[4]))
            y_C_all = np.append(y_C_all, float(big5_str[3]))
            y_E_all = np.append(y_E_all, float(big5_str[0]))
            y_A_all = np.append(y_A_all, float(big5_str[2]))
            y_N_all = np.append(y_N_all, float(big5_str[1]))
            old_id = user_id
        else:
            data_all[i].append(status)
